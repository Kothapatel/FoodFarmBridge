...
app.delete("/deleteProduct/:name", async (req, res) => {
  console.log("Delete request received for product:", req.params.name);
  const { name } = req.params;
  try {
    const result = await productModel.deleteOne({ name: name });
    if (result.deletedCount > 0) {
      console.log(`Product with name ${name} deleted.`);
      res.send({ message: `Product with name ${name} deleted successfully.` });
    } else {
      res.send({ message: `Product with name ${name} not found.` });
    }
  } catch (error) {
    console.log(error);
...
app.get("/products/category", async (req, res) => {
  const categories = ["vegetables", "fruits", "rice"];
  try {
    const data = await productModel.find({ category: { $in: categories } });
    console.log(data);
    res.send(data);
  } catch (error) {
    console.log(error);
